...
def model_predict(state, x, x0, t, ddpm_params, self_condition, is_pred_x0, cond, use_ema=True):
    if use_ema:
    ...
    if self_condition:
        pred = state.apply_fn(variables, jnp.concatenate([x, x0],axis=-1), t, cond)
    else:
        pred = state.apply_fn(variables, x, t, cond)
...
def ddpm_sample_step(state, rng, x, t, x0_last, cond, ddpm_params, self_condition=False, is_pred_x0=False):
 
    batched_t = jnp.ones((x.shape[0],), dtype=jnp.int32) * t
    
    if self_condition:
        x0, v = model_predict(state, x, x0_last, batched_t, ddpm_params, self_condition, is_pred_x0, cond, use_ema=True) 
    else:
        x0, v = model_predict(state, x, None, batched_t,ddpm_params, self_condition, is_pred_x0, cond, use_ema=True)
...
def sample_loop(rng, state, shape, p_sample_step, cond, timesteps):
    ...
    for t in reversed(jnp.arange(timesteps)):
        ...
        x, x0 = p_sample_step(state, step_rng, x, jax_utils.replicate(t), x0, cond)
        list_x0.append(x0)
    # normalize to [0,1]
