...
class Upsample(nn.Module):
  ...
  def __call__(self,x):
    ...
    dim = self.dim if self.dim is not None else C 
    if H==6:
        x= jax.image.resize(x, (B, H*2 +4, W*2 +4, C), 'nearest')
    else:
...
class DownUnet(nn.Module):
    ...
    def __call__(self, x, time, label):
        ...
        hs.append(h)
        cond= nn.Embed(self.dim, self.num_classes)(label)
        # use sinusoidal embeddings to encode timesteps
        time_emb = SinusoidalPosEmb(self.dim, dtype=self.dtype)(time)  # [B. dim]
        cond1= jnp.concatenate([time_emb,cond], axis=1)
        time_emb = nn.Dense(features=self.dim * 4, dtype=self.dtype, name='time_mlp.dense_0')(cond)
...
class QuanDownUnet(nn.Module):
    ...
    def __call__(self, x, time, label):
        ...
        cond1= jnp.concatenate([time_emb,cond], axis=1)
        time_emb = nn.Dense(features=self.dim * 4, dtype=self.dtype, name='time_mlp.dense_0')(cond1)
        time_emb = nn.Dense(features=self.dim * 4, dtype=self.dtype, name='time_mlp.dense_1')(nn.gelu(time_emb))  # [B, 4*dim]
...
class UNet(nn.Module):
    ...
    def __call__(self, x, time, label):
        ...
        Returns:
          x: jnp.ndarray of shape [B, H, W, C]
        """
        h, hs, time_emb= DownUnet(dim= self.dim, init_dim= self.init_dim, out_dim= self.out_dim, dim_mults= self.dim_mults, resnet_block_groups= self.resnet_block_groups, learned_variance= self.learned_variance, dtype= self.dtype, num_classes= self.num_classes, name='DownUnet')(x,time ,label)
...
class QVUNet(nn.Module):
    ...
    def __call__(self, x, time, label):
        ...
        Returns:
          x: jnp.ndarray of shape [B, H, W, C]
        """
        h, hs, time_emb= DownUnet(dim= self.dim, init_dim= self.init_dim, out_dim= self.out_dim, dim_mults= self.dim_mults, resnet_block_groups= self.resnet_block_groups, learned_variance= self.learned_variance, dtype= self.dtype,num_classes= self.num_classes, name='DownUnet1')(x,time,label)
