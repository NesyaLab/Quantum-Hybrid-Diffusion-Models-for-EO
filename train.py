...
def initialized(key, image_size,image_channel, model):
  ...
  variables = init(
      ...
      jnp.ones(input_shape[:1], dtype='int')#label
      )
...
def create_train_state(rng, config: ml_collections.ConfigDict):
  ...
  elif config.model.model == 'QVUNet':
     model = create_model(
        model_cls=unet.QVUNet, 
        half_precision=config.training.half_precision,
        dim = config.model.dim, 
        out_dim =  config.data.channels,
        dim_mults = config.model.dim_mults,
        quantum_channel= config.model.quantum_channel_vertex,
        name_ansatz= config.model.name_ansatz_vertex,
        num_layer= config.model.num_layer_vertex)
  elif config.model.model == 'FullQVUNet':
      model = create_model(
        model_cls=unet.FullQVUNet, 
        half_precision=config.training.half_precision,
        dim = config.model.dim, 
        out_dim =  config.data.channels,
        dim_mults = config.model.dim_mults,
        name_ansatz= config.model.name_ansatz_vertex,
        num_layer= config.model.num_layer_vertex)
  elif config.model.model == 'QuanvUNet':
      model = create_model(
        model_cls=unet.QuanvUNet, 
        half_precision=config.training.half_precision,
        dim = config.model.dim, 
        out_dim =  config.data.channels,
        dim_mults = config.model.dim_mults,
        quantum_channel_vertex= config.model.quantum_channel_vertex,
        name_ansatz_vertex= config.model.name_ansatz_vertex,
        num_layer_vertex= config.model.num_layer_vertex,
        quantum_channel_quan = config.model.quantum_channel_quan,
        name_ansatz_quan= config.model.name_ansatz_quan,
        num_layer_quan= config.model.num_layer_quan
        )
...
def create_train_state_for_tl(rng, state, config: ml_collections.ConfigDict):
  """Creates `TrainState` for transfer learing."""
  ...
  params['DownUnet1']= state.params['DownUnet']
  params['UpUnet1']= state.params['UpUnet']
...
# train step
def p_loss(rng, label,state, batch, ddpm_params, loss_fn, self_condition=False, is_pred_x0=False, pmap_axis='batch'):
    ...
    def compute_loss(params):
        pred = state.apply_fn({'params':params}, x_t, batched_t, label)
...
def save_checkpoint(state, workdir):
  if jax.process_index() == 0:
    ...
    step = int(state.step)
    checkpoints.save_checkpoint(workdir, state, step, keep=18)
...
def train(config: ml_collections.ConfigDict, 
  ...
  if config.training.transfer_learning: 
     num_steps_fin = config.training.num_train_steps_fin 
  ...
  for step, batch in zip(tqdm(range(step_offset, num_steps)), train_iter):
      ...
      if (step + 1) % config.training.save_and_sample_every == 0 or step + 1 == num_steps:
          ...
          for i in trange(0, config.training.num_sample, config.data.batch_size):
              ...
              cond= jnp.arange(config.data.batch_size)%(10)
              #cond= jnp.zeros(config.data.batch_size)
              #print(cond)
              cond= jnp.array(cond)
              cond=cond.reshape(1,-1)
              samples.append(sample_loop(sample_rng, state, tuple(batch['image'].shape), p_sample_step, cond, config.ddpm.timesteps))
          samples = jnp.concatenate(samples) # num_devices, batch, H, W, C
          sample_1 = np.asarray(samples)

          this_sample_dir = os.path.join(sample_dir, f"iter_{step}_host_{jax.process_index()}")
          os.makedirs(this_sample_dir, exist_ok=True)
          file_path = os.path.join(this_sample_dir, "images.npy")

          # Salvataggio dell'array
          np.save(file_path, sample_1)
          tf.io.gfile.makedirs(this_sample_dir)
          
          with tf.io.gfile.GFile(
              os.path.join(this_sample_dir, "sample.png"), "wb") as fout:
            samples_array = utils.save_image(samples, 100, fout, padding=2)
  ...
  if config.training.transfer_learning:
     ...
     state1= create_train_state_for_tl(state_rng,state, config)
     state1= jax_utils.replicate(state1)

     for step, batch in zip(tqdm(range(num_steps, num_steps_fin)), train_iter):
      batch= batch['image']
      label1 = batch['label']
      label=[]
      for i in range(len(label1[0])):
      ...
      rng, *train_step_rng = jax.random.split(rng, num=jax.local_device_count() + 1)
      train_step_rng = jnp.asarray(train_step_rng)
      state1, metrics = p_train_step(train_step_rng, label,state1, batch)
      for h in hooks:
          h(step)
      if step == step_offset:
          logging.info('Initial compilation completed.')
      ...
      # update state.params_ema
      if (step + 1) <= config.ema.update_after_step:
          state1 = p_copy_params_to_ema(state1)
      elif (step + 1) % config.ema.update_every == 0:
          ema_decay = ema_decay_fn(step)
          logging.info(f'update ema parameters with decay rate {ema_decay}')
          state1 =  p_apply_ema(state1, ema_decay)
      ...
      if (step + 1) % config.training.save_and_sample_every == 0 or step + 1 == num_steps:
          ...
          for i in trange(0, config.training.num_sample, config.data.batch_size):
              ...
              cond=cond.reshape(1,-1)
              rng, sample_rng = jax.random.split(rng)
              samples.append(sample_loop(sample_rng, state1, tuple(batch['image'].shape), p_sample_step,cond, config.ddpm.timesteps))
          samples = jnp.concatenate(samples) # num_devices, batch, H, W, C
          ...
          with tf.io.gfile.GFile(
            ...
            if config.wandb.log_sample:
                utils.wandb_log_image(samples_array, step+1)
          # save the chceckpoint
          save_checkpoint(state1, workdir)
          if step + 1 == num_steps and config.wandb.log_model:
              utils.wandb_log_model(workdir, step+1)
